...
async def test_list_backups_success(s3_manager):
    ...
    with patch.object(manager, "executor"):
        with patch("asyncio.get_event_loop") as mock_get_loop:
            mock_loop = AsyncMock()
            mock_get_loop.return_value = mock_loop
...
async def test_restore_backup_success(s3_manager, tmp_path):
    ...
    get_object_responses = {
        ...
        "hdev-memory-backups/test-backup/entries/entry1.json.gz": {
            "Body": MagicMock(
                read=lambda: gzip.compress(
                    json.dumps(
                        {"content": "Entry 1 content", "metadata": {"type": "test"}}
                    ).encode()
                )
            )
        },
        "hdev-memory-backups/test-backup/entries/entry2.json.gz": {
            "Body": MagicMock(
                read=lambda: gzip.compress(
                    json.dumps(
                        {"content": "Entry 2 content", "metadata": {"type": "test"}}
                    ).encode()
                )
            )
        },
    ...
    with patch.object(manager, "executor"):
        with patch("asyncio.get_event_loop") as mock_get_loop:
            ...
            result = await manager.restore_backup(
                target_backend, "test-backup", overwrite=False
            )
...
async def test_collect_entry_paths(s3_manager):
    ...
    tree = {
        ...
        "projects": {"project1": {}, "frontend": {"react": {}}},
        "personal": {"notes": {}},
    }
