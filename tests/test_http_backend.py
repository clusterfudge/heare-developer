...
def http_backend():
    ...
    return HTTPMemoryBackend(
        base_url="https://memory.example.com", api_key="test-key", timeout=10
    )
...
async def test_write_entry(http_backend):
    ...
    with patch.object(http_backend, "_make_request") as mock_request:
        ...
        result = await http_backend.write_entry(
            "test/entry", "Test content", {"version": 1}
        )
...
async def test_search(http_backend):
    ...
    mock_response_data = {
        "query": "test",
        "results": [{"path": "test/entry", "snippet": "Test content", "score": 0.9}],
        "success": True,
        "error": None,
    }
...
async def test_retry_logic():
    ...
    backend = HTTPMemoryBackend(base_url="https://memory.example.com", max_retries=2)
