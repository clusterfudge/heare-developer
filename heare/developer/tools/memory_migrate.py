...
from heare.developer.config import get_config
...
class MemoryMigrator:
    ...
    async def migrate_all(
        ...
        """
        logger.info(
            f"Starting memory migration (dry_run: {dry_run}, overwrite: {overwrite})"
        )
            ...
            try:
                ...
                if result["success"]:
                    ...
                    self.stats["errors"].append(
                        {"path": entry_path, "error": result["error"]}
                    )
            except Exception as e:
                self.stats["failed_entries"] += 1
                self.stats["errors"].append({"path": entry_path, "error": str(e)})
    ...
    async def _collect_entries_from_tree(
        self, tree: Dict, prefix: str, entries: List[str]
    ):
    ...
    async def _migrate_entry(
        self, entry_path: str, overwrite: bool, dry_run: bool
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            write_result = await self.target_backend.write_entry(
                entry_path, source_result["content"], source_result["metadata"]
            )
...
async def backup_memory_s3(
    context: AgentContext, backup_name: Optional[str] = None
) -> str:
    """Backup memory to S3.
    ...
    try:
        ...
        # Run backup
        result = await s3_manager.backup_all(
            context.memory_manager.backend, backup_name
        )
...
async def restore_memory_s3(
    context: AgentContext, backup_key: str, overwrite: bool = False
) -> str:
    ...
    try:
        ...
        result = await s3_manager.restore_backup(
            context.memory_manager.backend, backup_key, overwrite
        )
        ...
        if result["success"]:
            ...
            if result.get("backup_metadata"):
                ...
                output.append(
                    f"Original backend: {metadata.get('backend_type', 'unknown')}"
                )
...
async def _parse_backend_config(config_str: str):
    ...
    if config_str == "filesystem":
        ...
        raise ValueError(
            f"Unknown backend config: {config_str}. Use 'filesystem' or 'http:URL'"
        )
...
def migrate_memory_cli_main(args: List[str]):
    ...
    parser.add_argument("--source-url", help="URL for source HTTP backend")

    parser.add_argument("--target-url", help="URL for target HTTP backend")
        ...
        try:
            ...
            if parsed_args.source == "filesystem":
                ...
                if not parsed_args.source_url:
                    ...
                    return
                source_backend = HTTPMemoryBackend(
                    base_url=parsed_args.source_url, api_key=api_key
                )
            ...
            if parsed_args.target == "filesystem":
                ...
                if not parsed_args.target_url:
                    print("Error: --target-url is required for HTTP target")
                    return
                target_backend = HTTPMemoryBackend(
                    base_url=parsed_args.target_url, api_key=api_key
                )
            ...
            result = await migrator.migrate_all(
                overwrite=parsed_args.overwrite, dry_run=parsed_args.dry_run
            )
...
def backup_memory_cli_main(args: List[str]):
    ...
    parser = argparse.ArgumentParser(
        prog="hdev backup-memory", description="Backup memory to S3"
    )

    parser.add_argument(
        "--backup-name", help="Custom backup name (default: timestamp-based)"
    )

    parser.add_argument(
        "--s3-bucket", help="S3 bucket for backup (can also use HDEV_S3_BUCKET env var)"
    )
    ...
    parser.add_argument(
        "--s3-endpoint-url", help="S3 endpoint URL for S3-compatible services"
    )
    ...
    parser.add_argument("--memory-url", help="URL for HTTP memory backend")
    ...
    async def run_backup():
        try:
            ...
            s3_config = MemoryConfig(
                ...
                s3_endpoint_url=parsed_args.s3_endpoint_url
                or os.getenv("HDEV_S3_ENDPOINT_URL"),
            )

            if not s3_config.s3_bucket:
                print(
                    "Error: S3 bucket must be specified via --s3-bucket or HDEV_S3_BUCKET env var"
                )
                return
            ...
            if parsed_args.memory_backend == "filesystem":
                ...
                backend = HTTPMemoryBackend(
                    base_url=parsed_args.memory_url, api_key=api_key
                )
...
def restore_memory_cli_main(args: List[str]):
    ...
    parser = argparse.ArgumentParser(
        prog="hdev restore-memory", description="Restore memory from S3 backup"
    )

    parser.add_argument("backup_key", help="Backup key to restore from")
    ...
    async def run_restore():
        try:
            ...
            # Run restore
            result = await s3_manager.restore_backup(
                backend, parsed_args.backup_key, parsed_args.overwrite
            )
