...
class ConfigManager:
        ...
        if self.config_path.exists():
            try:
                ...
                memory_config = MemoryConfig(
                    ...
                    filesystem_path=Path(memory_data["filesystem"]["path"])
                    if memory_data.get("filesystem", {}).get("path")
                    else None,
                    ...
                    s3_secret_access_key=memory_data.get("s3", {}).get(
                        "secret_access_key"
                    ),
    ...
    def _apply_env_overrides(self):
        ...
        if os.getenv("HDEV_MEMORY_FILESYSTEM_PATH"):
            self._config.memory.filesystem_path = Path(
                os.getenv("HDEV_MEMORY_FILESYSTEM_PATH")
            )
        ...
        if os.getenv("HDEV_MEMORY_HTTP_TIMEOUT"):
            try:
                self._config.memory.http_timeout = int(
                    os.getenv("HDEV_MEMORY_HTTP_TIMEOUT")
                )
            except ValueError:
                pass
        ...
        if os.getenv("HDEV_S3_SECRET_ACCESS_KEY"):
            self._config.memory.s3_secret_access_key = os.getenv(
                "HDEV_S3_SECRET_ACCESS_KEY"
            )
    ...
    def save_config(self, config: HdevConfig):
        ...
        data = {
            "memory": {
                ...
                "filesystem": {
                    "path": str(config.memory.filesystem_path)
                    if config.memory.filesystem_path
                    else None
                },
                "http": {
