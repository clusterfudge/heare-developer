...
class S3BackupManager:
    ...
    def __init__(self, config: MemoryConfig):
        ...
        if not S3_AVAILABLE:
            raise ImportError(
                "boto3 is required for S3 backup functionality. Install with: pip install boto3"
            )
    ...
    async def backup_all(
        self, backend: MemoryBackend, backup_name: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def restore_backup(
        self, backend: MemoryBackend, backup_key: str, overwrite: bool = False
    ) -> Dict[str, Any]:
                ...
                try:
                    ...
                    if entry_data:
                        result = await backend.write_entry(
                            entry_path, entry_data["content"], entry_data["metadata"]
                        )
                        if result["success"]:
                            success_count += 1
                        else:
                        ...
                        errors.append(
                            {
                                "path": entry_path,
                                "error": "Failed to download entry data",
                            }
                        )
        ...
        try:
            ...
            response = await loop.run_in_executor(
                ...
                lambda: self.s3_client.list_objects_v2(
                    Bucket=self.bucket, Prefix="hdev-memory-backups/", Delimiter="/"
                ),
            )
    ...
    async def delete_backup(self, backup_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await loop.run_in_executor(
                self.executor,
                lambda: self.s3_client.list_objects_v2(
                    Bucket=self.bucket, Prefix=f"hdev-memory-backups/{backup_key}/"
                ),
            )
    ...
    async def _collect_all_entries(
    ...
    async def _backup_entry(
        self, backup_name: str, entry_path: str, entry_data: Dict[str, Any]
    ):
    ...
    async def _get_backup_metadata(
        self, backup_key: str, silent: bool = False
    ...
    async def _download_entry(
        self, backup_key: str, entry_path: str
    ) -> Optional[Dict[str, Any]]:
